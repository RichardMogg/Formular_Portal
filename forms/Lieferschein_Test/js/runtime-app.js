...
(function () {
  ...
  function ensureZipExportButton() {
    ...
    var exportsConfig = protocols.exports;
    var hasAnyExport = exportsConfig && (
      exportsConfig.json !== false ||
      exportsConfig.csv !== false ||
      exportsConfig.pdf !== false ||
      exportsConfig.html !== false ||
      exportsConfig.images !== false
    );
    if (!hasAnyExport) return;
    var buttonRow = document.querySelector('.preview-shell.runtime-form-shell > .button-row');
    if (!buttonRow || document.getElementById('runtimeExportZipButton')) return;
    var button = document.createElement('button');
    button.type = 'button';
    button.className = 'btn-secondary';
    button.id = 'runtimeExportZipButton';

    // Determine active formats to set optimal button text
    var config = exportsConfig || { json: true, csv: true, pdf: true, html: true, images: true };
    var activeFormats = [];
    if (config.json !== false) activeFormats.push('json');
    if (config.csv !== false) activeFormats.push('csv');
    if (config.pdf !== false) activeFormats.push('pdf');
    if (config.html !== false) activeFormats.push('html');
    if (config.images !== false) activeFormats.push('images');

    if (activeFormats.length === 1) {
      var single = activeFormats[0];
      if (single === 'json') button.textContent = 'Protokolle als JSON exportieren';
      else if (single === 'csv') button.textContent = 'Protokolle als CSV exportieren';
      else if (single === 'pdf') button.textContent = 'Protokolle als PDF exportieren';
      else if (single === 'html') button.textContent = 'Protokolle als HTML exportieren';
      else if (single === 'images') button.textContent = 'Fotos/Dateien exportieren';
      button.title = 'Erzeugt und lädt alle Protokolle direkt in diesem Format herunter.';
    } else {
      button.textContent = 'Protokolle als ZIP exportieren';
      button.title = 'Erzeugt und lädt ein ZIP-Archiv mit allen Protokollen herunter.';
    }
    button.addEventListener('click', function () {
      if (window.FormRuntimeZipExportActions && typeof window.FormRuntimeZipExportActions.exportZip === 'function') {
        window.FormRuntimeZipExportActions.exportZip();
      } else {
        window.alert('ZIP-Export-Modul nicht geladen.');
      }
    });
  ...
  function init(){cfg=window[CONFIG_NAME];if(!cfg||!cfg.schema)return;schema=cfg.schema;protocolMode();cache();applyRuntimeDesign();renderRuntimeAssets();state=loadState();data=state.draft&&state.draft.data?clone(state.draft.data):createInitialFormData(schema);header();renderForm();renderList();bindEvents();ensureZipExportButton();setStatus('Runtime-App geladen.','ok');var runtimePrintActionsScript=document.createElement('script');runtimePrintActionsScript.src='js/runtime-print-actions.js';document.body.appendChild(runtimePrintActionsScript);}
  window.FormRuntimeApp={init:init,getConfig:function(){return cfg;},getSchema:function(){return schema;},getState:function(){return state;},getFormData:function(){return data;},getEditingId:function(){return editingId;},renderForm:renderForm,renderProtocolList:renderList};
