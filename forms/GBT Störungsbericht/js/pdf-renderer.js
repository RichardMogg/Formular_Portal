...
function rasterizePrintBackgroundForPdf(holder) {
  ...
  // Parse width/height from viewBox or default to high-res 1200/1200
  ...
  return svgToPngDataUrl(svg, targetWidth, targetHeight).then(function (pngDataUrl) {
    if (!pngDataUrl) {
      return;
    }
    ...
    image.alt = '';
    // Styling will be handled via the CSS stylesheet rule matching both svg and img
    watermark.appendChild(image);
  });
}

function rasterizeHeaderLogoForPdf(holder) {
  var logoWrap = holder.querySelector('.print-logo');
  var svg = logoWrap ? logoWrap.querySelector('svg') : null;

  if (!logoWrap || !svg) {
    return Promise.resolve();
  }

  // Parse width/height from viewBox or default to high-res 1200/400
  var targetWidth = 1200;
  ...
  return svgToPngDataUrl(svg, targetWidth, targetHeight).then(function (pngDataUrl) {
    ...
    image.src = pngDataUrl;
    image.alt = '';
    // Sizing handled perfectly by print logo styling
...
function preparePdfDomForCanvas(holder) {
  return rasterizePrintBackgroundForPdf(holder).then(function () {
    return rasterizeHeaderLogoForPdf(holder);
  }).then(function () {
