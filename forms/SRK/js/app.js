...
'</div>';
  ...
  photoInput.addEventListener('change', function () {
    updateAttachmentListFromInput('rackkuehlgeraet', photoInput, unitId);
  });
...
function collectProtocol() {
  var data = {
    ...
    pruefung: {
      ...
      rackkuehlgeraete: collectIndoorUnits(),
...
unterschrift: {
  ...
  data.pruefung.inneneinheiten = data.pruefung.rackkuehlgeraete;
...
function collectPhotoMeta() {
  var rackkuehlgeraete = [];

  document.querySelectorAll('.indoor-card').forEach(function (card, index) {
    var unitId = card.getAttribute('data-unit-id');

    rackkuehlgeraete.push({
      id: unitId,
      nummer: index + 1,
      bezeichnung: getInputValue(card.querySelector('[data-rk-field="bezeichnung"]')),
      fotos: makeAttachmentMeta(currentIndoorPhotos[unitId] || [])
  ...
  return {
    ...
    rackkuehlgeraete: rackkuehlgeraete
  };
}
...
function fillFormFromProtocol(data) {
  ...
  var units = pruefung.rackkuehlgeraete || pruefung.rueckkuehlgeraete || pruefung.inneneinheiten || [];
...
function getProtocolValidationIssues(data, label) {
  ...
  requireValue(issues, prefix, kopfdaten.anlagenerrichter, 'Anlagenerrichter fehlt');
  ...
  (pruefung.rackkuehlgeraete || []).forEach(function (unit, index) {
...
function editProtocol(index) {
  ...
  if (bundle) {
    ...
    (bundle.rackkuehlgeraete || bundle.rueckkuehlgeraete || []).forEach(function (unit) {
...
function getPhotoExportIssues() {
  ...
  appState.protocols.forEach(function (record, index) {
    ...
    (meta.rackkuehlgeraete || []).forEach(function (unit) {
      var found = null;

      if (bundle) {
        (bundle.rackkuehlgeraete || []).forEach(function (b) {
          if (b.id === unit.id) {
            found = b;
          }
        });
...
function addAttachmentBundleToZip(files, folder, bundle) {
  ...
  (bundle.rackkuehlgeraete || []).forEach(function (unit, unitIndex) {
    var unitFolder = folder +
      'fotos-dateien/rackkuehlgeraete/rackkuehlgeraet_' +
      pad3(unitIndex + 1) +
      '_' +
...
function buildCsvForProtocols(records) {
  ...
  records.forEach(function (record) {
    ...
    (p.rackkuehlgeraete || []).forEach(function (unit, index) {
...
function addCsvAttachmentMetaRows(rows, recordId, fotos) {
  ...
  (fotos.rackkuehlgeraete || []).forEach(function (u) {
...
var rackUnits = p.rackkuehlgeraete || p.rueckkuehlgeraete || p.inneneinheiten || [];
...
html += '</div>';
  ...
  html += printFieldTable('Inbetriebnahmeergebnis', p.inbetriebnahmeergebnis || {}, FIELD_GROUPS.ergebnis, false);
  html += printFieldTable('Anlagendokumentation / Einweisung', p.dokumentation || {}, FIELD_GROUPS.dokumentation, false);
...
function importJsonFromFile(event) {
  ...
  reader.onload = function () {
    try {
      ...
      saveState();
      renderProtocolList();
      setStatus(imported.length + ' Protokoll(e) importiert. Fotodateien und Betreiber-Unterschriften werden durch JSON nicht wiederhergestellt.', 'ok');
      openSection('sectionListe', true);
    } catch (err) {
...
return source.map(function (item) {
  var data = removeBetreiberSignatureOnJsonImport(item.data || item);
...
function removeBetreiberSignatureOnJsonImport(data) {
  if (!data) {
    return data;
  }
  ...
  if (cleanData.unterschrift) {
    cleanData.unterschrift.betreiberVorhanden = false;
    cleanData.unterschrift.betreiberDataUrl = '';
  }
